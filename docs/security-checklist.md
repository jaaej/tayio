...
| B3 | Rate limiting on `/login` (brute-force protection) | ⚠ | P0 | **Implemented 2026-07-02, NEEDS LIVE TEST.** Login moved from client-side `signInWithPassword` to a server action (`src/app/(auth)/login/actions.ts`) wrapped in a Postgres-backed rate limiter (migration 0014, `check_rate_limit`): 20/5min per IP + 5/15min per email. Limiter function verified (fixed-window + reset PASS). **Not yet E2E-tested: the server-side sign-in + cookie + redirect flow must be clicked through before trusting** (blast radius = login). Free-tier; no Pro Auth Hooks needed |
...
| F3 | Content-Security-Policy header (nonce-based) | ⚠ | P1 | **Upgraded to nonce-based 2026-07-02, NEEDS LIVE TEST.** CSP now built per-request in middleware (`src/lib/security-headers.ts:buildCsp`) with a random nonce; forwarded on the request header so Next stamps its inline scripts. **Prod drops `'unsafe-inline'` for scripts** → `script-src 'self' 'nonce-…' 'strict-dynamic'` (real XSS control). Dev stays `'unsafe-inline' 'unsafe-eval'` for HMR. All other directives locked (`default-src/object-src/base-uri/frame-ancestors/form-action`, scoped img/media/connect). `style-src 'unsafe-inline'` still required by inline `style={{}}`. No custom inline scripts in the app. **Blast radius = every page; must be click-tested in a prod-mode build (`next build && next start`) before trusting — a bad CSP blocks all scripts.** Static headers (HSTS/XFO/nosniff/Referrer/Permissions) remain in `next.config.ts` |
