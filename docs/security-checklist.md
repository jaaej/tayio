...
| A13 | RLS enabled on quizzes tables | ✓ | P0 | **Runtime-verified 2026-07-27.** Migrations 0025 and 0026 cover `quizzes`, `quiz_questions`, `quiz_options`, and `quiz_attachments`. Admins have full access; assigned tutors retain lifecycle write access; tutors may read approved quizzes for taught subjects. Students may discover approved quiz rows and attachments for enrolled subjects, but have no direct question/option policy because options contain `is_correct`. The student server query checks active enrolment and omits answer-key fields before delivery. Attachment mutations are app-authorized and audited. `npm run db:check-rls` passes all 37 public tables against the connected database. Migration 0027 added `quiz_questions.parent_id` (self-referential, for context/passage blocks) and `quiz_attachments.question_id` (per-question attachments); both are covered by the existing `quiz_id`-keyed policies with no new policies and no widened access, verified by `db:check-rls` on 2026-07-27. |
| A14 | RLS + server-side guards on term-test scoring/leaderboard tables | ⚠ | P0 | **Code-reviewed and machine-verified 2026-07-29; owner browser confirmation still pending.** Migration `0028_term_test.sql` adds `term_test_attempts`/`term_test_answers` with RLS enabled; migration `0029_term_test_rls_tighten.sql` (also applied) tightens the student policy on both from `for all` to SELECT-only, because every write goes through server code (`submitTermTest`, via Drizzle, which bypasses RLS) - left at `for all`, a student could otherwise self-insert a fake perfect-score attempt directly via PostgREST with their own JWT, corrupting the leaderboard. Five checks, each verified by reading the code: (1) the take-test payload never includes `is_correct` - `getStudentTermTest`'s (`src/lib/quiz-queries.ts`) option projection selects only `id/questionId/text/position`; (2) grading is server-only - `submitTermTest` (`src/app/_actions/term-tests.ts`) loads the answer key and calls the pure `gradeTermTest` (`src/lib/term-test.ts`) itself, the client never submits a score; (3) one attempt per student is enforced by the `unique(quiz_id, student_id)` constraint on `term_test_attempts` AND an application-level catch of the resulting `23505` (`isUniqueViolation`, `src/lib/db-errors.ts`) that returns a clean "already taken" error instead of a 500; (4) results (score, rank, corrections, board) are embargoed until `results_release_at` - `getStudentTermTestResults`/`getParentTermTestResults` (`src/lib/term-test-results.ts`) check the release gate before touching any of that data, and the `submitted_pending` UI state never calls those functions; (5) parent access is gated by both `familyLinks` (parent-child link) and cohort membership (`getTermTestCohort`: active enrollment in a class of the subject at the deadline) - checked in that order, before the release gate, so an unlinked parent or an out-of-cohort child learns nothing about whether the test exists. Also verified: `getStudentQuiz` and `gradePracticeQuiz` (the weekly practice route and grader) both reject `kind !== "weekly"`, so an approved term test cannot be opened or graded through the untouched weekly path and leak its answer key. `npm run db:check-rls` is green for both new tables after 0029; `npm test` passes 17 term-test unit tests covering grading boundaries, all four embargo states, and no-show ranking. **Not yet done:** an owner browser session confirming `is_correct` never appears in a real take-test network request, that a second submit attempt is actually blocked end-to-end, and that the parent/student pending-then-released transition behaves correctly against the live database - hence ⚠ rather than ✓. See `docs/changes/2026-07-29-term-test-leaderboard.md`. |
