...
| Quiz management | Request, build, approve quizzes | 🔶 | 🔶 | **Quiz maker shipped on feat/quiz-maker (2026-07-26).** Admin creates quiz request → tutor builds → admin approves. Pages `/admin/quizzes` (list + request creation + approval actions). Backend: `requestQuiz`, `createQuizDirect`, `approveQuiz`, `requestChanges` in `src/app/_actions/quizzes.ts`. Migration 0025 (RLS + `quizzes`/`quiz_questions`/`quiz_options` tables) applied by owner. **Pending runtime click-through.** **Still unbuilt:** student quiz-taking UI, student quiz ranking/analytics. |
