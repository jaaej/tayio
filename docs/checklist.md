...
| Lesson plan | Update what's *going* to be covered | ⬜ | ⬜ | `lesson_notes.nextLessonFocus` exists (per-lesson, *retroactive* - "what to focus on next time"). A `classes.lesson_plan` column exists (`src/db/schema.ts:169`) but is dead - no read/write/UI anywhere - so the forward-looking class-level plan is not built as a feature (2026-08-01 audit). |
