...
| End-of-term term test + separate leaderboard | 🔶 | 🔶 | **Authoring UI built 2026-07-29 (task 6/9 of the term-test-leaderboard plan).** A term test is a `quizzes` row with `kind='term_test'`, authored through the same quiz maker + approval flow as weekly quizzes, scoped to a term instead of a week. Admin can create one at `/admin/quizzes` (`NewTermTestForm` - subject+term slot, title, optional tutor assignment; release date defaults to the term's end date and is not settable at creation) and both `/admin/quizzes` and `/tutor/quizzes` now group rows under a "Term tests" heading with kind-aware "Term N test" labels instead of "Week N". The shared `quiz-maker.tsx` header is kind-aware and shows an admin-only inline release-date editor (`setTermTestReleaseDate`, guarded to admin + unreleased + `kind='term_test'`) once a test exists. Backend (schema, `createTermTest`/`setTermTestReleaseDate`/`submitTermTest` actions, kind-aware queries, server-side grading) and this authoring UI's typecheck/build/62-test suite are verified; **no owner browser click-through yet**, so FE stays 🔶. Still unbuilt: the ranked leaderboard itself (separate from weekly quiz practice) and any student-facing term-test taking UI beyond the already-built `submitTermTest` action, so BE stays 🔶 for the row as a whole. Also fixed in this task: `isUniqueViolation` had been exported from the `"use server"` file `src/app/_actions/quizzes.ts` as a synchronous function, which fails `next build`'s "Server Actions must be async" check; moved it to `src/lib/db-errors.ts` (a plain, non-"use server" helper) and updated both `quizzes.ts` and `term-tests.ts` to import from there. |
