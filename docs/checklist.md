...
| End-of-term term test + separate leaderboard | 🔶 | 🔶 | **Student take/pending/results UI + leaderboard built 2026-07-29 (task 7/9 of the term-test-leaderboard plan)**, on top of the admin/tutor authoring UI from task 6. A term test is a `quizzes` row with `kind='term_test'`, scoped to a term instead of a week. Student-facing state machine lives at `/student/term-tests/[id]` (`src/app/student/term-tests/[id]/page.tsx`): `requireRole("student")` loads `getStudentTermTest`, derives state via `deriveTermTestState`, and renders one of four sub-components under `_components/` - `take-form.tsx` (single-select per question, context-set passages, a two-step "one attempt only" confirm before calling `submitTermTest`, then `router.refresh()`), `status-card.tsx` (shared calm card for `not_open` and `submitted_pending` - the pending state renders only a release-date message, never touching `getStudentTermTestResults`, honouring the embargo), and `results-view.tsx` (score/rank hero + per-question corrections - answer marked right/wrong, correct answer shown, no explanation text - + the leaderboard), fed by `getStudentTermTestResults` only once state is `released`. New `src/components/term-test/leaderboard.tsx` adapts the math-game leaderboard's Row markup/medals/"you" styling into a single board (no difficulty tabs), titled by subject + term. The student subject page (`/student/subjects/[id]`) now surfaces a `TermTestCard` (`_components/term-test-card.tsx`) above the week view - visually and structurally separate from the per-week practice-quiz card - linking to `/student/term-tests/{id}` with a Take/Submitted/Results label; backed by a new minimal `getStudentTermTestForSubjectTerm(studentId, subjectId, termId)` lookup in `src/lib/quiz-queries.ts` (reuses `canStudentAccessApprovedQuiz` for the enrolment check) since no such helper existed yet. **Parent read-only results + leaderboard view built 2026-07-29 (task 8/9)**: `/parent/term-tests/[id]/[childId]` (`src/app/parent/term-tests/[id]/[childId]/page.tsx`) calls `requireRole("parent")` then `getParentTermTestResults(parent.id, childId, id)` - the only child-scoped call on the page, enforcing the family-link + cohort guard - and `notFound()`s on null; before release it reuses the student's `TermTestStatusCard` verbatim (embargo: no score/rank/corrections/board reach the page at all pre-release); once released it reuses the student's `TermTestResultsView` and `TermTestLeaderboard` directly rather than forking near-duplicates, via one new optional prop each - `TermTestResultsView`'s `childName` switches "Your answer"/the results eyebrow to third person, and `TermTestLeaderboard`'s `meLabel` (default `"you"`) replaces the highlighted row's "(you)" tag with the child's name, since the board's "me" row is the child, not the parent. Subject/term labels for the header and the "Back to subject" link come from a new non-child-scoped `getTermTestSubjectAndTerm(quizId)` in `quiz-queries.ts`, called only after the access gate passes. Entry point: the parent subject page (`/parent/subjects/[id]`) now surfaces a `ParentTermTestCard` (`_components/term-test-card.tsx`) above the week view, but only once results are released (`getParentCurriculum` computes `termTestResultsId` by reusing `getStudentTermTestForSubjectTerm(childId, ...)` as-is - a child is a student, and the family link is already checked earlier in the same function, so no parallel lookup was needed). Typecheck and `next build` both pass clean (exit 0); **no owner browser click-through yet**, so FE and BE stay 🔶 for the row as a whole. |
