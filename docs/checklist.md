...
| Item | FE | BE | Notes |
|---|---|---|---|
| Cancellation -> class credit | 🔶 | ✅ | Cancelling a class grants a class credit. **Built (2026-07-30), migration 0031.** BE: `cancelLesson` in `src/lib/credits.ts` (24h notice + per-term cap + double-cancel/double-move guard via `lessonCancellations` + `rescheduleRequests`), wired through `src/app/_actions/credits.ts`. FE: student `src/app/student/_components/interactive-timetable.tsx` (cancel-confirm popover) and parent `src/components/reschedule/reschedule-form.tsx` (`CancelLessonAction`, its own card on `/parent/classes/reschedule/[lessonId]`). Both submit the child/own `studentId`; parent linkage re-verified server-side. BE machine-verified (typecheck, unit tests, build, `db:check-rls`); FE partial pending owner browser QA - see `docs/changes/2026-07-29-reschedule-credits.md`. |
| Reschedule -> class credit when no slot | 🔶 | ✅ | If there is no class to reschedule into, grant a class credit instead. **Built (2026-07-30), migration 0031.** BE: `grantRescheduleCredit` in `src/app/_actions/reschedule.ts` (7-day notice + per-term cap, counts toward the reschedule cap). FE: student `interactive-timetable.tsx` "Get a class credit instead" button in the no-slots state; parent `reschedule-form.tsx` mirrors it in the same empty-slots branch. BE machine-verified; FE partial pending owner browser QA - see `docs/changes/2026-07-29-reschedule-credits.md`. |
| Reschedule/cancellation limits | 🔶 | ✅ | Cancellation: 24h prior, max 3 per term. Reschedule: 1 week prior, max 3 per term (counted separately from cancellations). **Built (2026-07-30), migration 0031.** Pure logic in `src/lib/reschedule-credits.ts` (`CANCEL_CAP`/`RESCHEDULE_CAP` = 3, `meetsCancelNotice`/`meetsRescheduleNotice`, `resolveTerm`, `remaining`), enforced server-side in `src/lib/credits.ts` + `src/app/_actions/reschedule.ts`. FE allowance labels ("N of 3 left") on both student (`interactive-timetable.tsx` action menu) and parent (`reschedule-form.tsx` calendar header, `CancelLessonAction` trigger button); parent page also shows a specific ineligibility reason (no term / notice / cap) in place of the picker when not eligible. **[DECISION] resolved 2026-07-29:** the 7-day figure is a notice deadline (the reschedule/makeup must be initiated at least 7 days before the original lesson), not a separate landing-window constraint - the makeup itself can land in any available same-tutor slot as before. Notice windows govern self-serve; caps are 3 cancellations + 3 reschedules per term, counted independently; admin-initiated cancels/reschedules are uncapped and never counted. BE machine-verified; FE partial pending owner browser QA - see `docs/changes/2026-07-29-reschedule-credits.md`. |
