...
| Item | FE | BE | Notes |
|---|---|---|---|
| Cancellation -> class credit | ✅ | ✅ | Cancelling a class grants a class credit. **Built (2026-07-30).** BE: `cancelLesson` in `src/lib/credits.ts` (24h notice + per-term cap + double-cancel/double-move guard via `lessonCancellations` + `rescheduleRequests`), wired through `src/app/_actions/credits.ts`. FE: student `src/app/student/_components/interactive-timetable.tsx` (cancel-confirm popover) and parent `src/components/reschedule/reschedule-form.tsx` (`CancelLessonAction`, its own card on `/parent/classes/reschedule/[lessonId]`). Both submit the child/own `studentId`; parent linkage re-verified server-side. Typecheck + build pass; browser verification owner-pending. |
| Reschedule -> class credit when no slot | ✅ | ✅ | If there is no class to reschedule into, grant a class credit instead. **Built (2026-07-30).** BE: `grantRescheduleCredit` in `src/app/_actions/reschedule.ts` (7-day notice + per-term cap, counts toward the reschedule cap). FE: student `interactive-timetable.tsx` "Get a class credit instead" button in the no-slots state; parent `reschedule-form.tsx` mirrors it in the same empty-slots branch. |
| Reschedule/cancellation limits | ✅ | ✅ | Cancellation: 24h prior, max 3 per term. Reschedule: 1 week prior, max 3 per term (counted separately from cancellations). **Built (2026-07-30).** Pure logic in `src/lib/reschedule-credits.ts` (`CANCEL_CAP`/`RESCHEDULE_CAP` = 3, `meetsCancelNotice`/`meetsRescheduleNotice`, `resolveTerm`, `remaining`), enforced server-side in `src/lib/credits.ts` + `src/app/_actions/reschedule.ts`. FE allowance labels ("N of 3 left") on both student (`interactive-timetable.tsx` action menu) and parent (`reschedule-form.tsx` calendar header, `CancelLessonAction` trigger button); parent page also shows a specific ineligibility reason (no term / notice / cap) in place of the picker when not eligible. Group-class approval path is retired - every reschedule is this direct notice+cap model. |
