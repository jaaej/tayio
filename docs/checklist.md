...
| Quiz management | Request, build, approve quizzes | 🔶 | 🔶 | **Quiz maker shipped on feat/quiz-maker (2026-07-26).** Admin creates quiz request → tutor builds → admin approves. Pages `/admin/quizzes` (list + request creation + approval actions). Backend: `requestQuiz`, `createQuizDirect`, `approveQuiz`, `requestChanges` in `src/app/_actions/quizzes.ts`. Migration 0025 (RLS + `quizzes`/`quiz_questions`/`quiz_options` tables) applied + verified 2026-07-26 (RLS enabled on all 3, 2 policies each, enums 5+2). **Pending runtime click-through.** **Still unbuilt:** student quiz-taking UI, student quiz ranking/analytics. |
...
| Create quizzes | Build quizzes from admin requests | 🔶 | 🔶 | **Quiz maker shipped on feat/quiz-maker (2026-07-26).** Tutor page `/tutor/quizzes` lists quiz requests and allows tutor to build (multiple-choice + true/false formats). Quiz builder UI (`src/components/quiz/quiz-maker.tsx`, `QuizMaker` component). Backend: `submitQuiz` action in `src/app/_actions/quizzes.ts`. Migration 0025 applied + verified 2026-07-26. **Pending runtime click-through.** **Still unbuilt:** student quiz-taking, quiz ranking/analytics. |
...
| Take quizzes | Complete assigned quizzes | ⬜ | ⬜ | Not built. Requires student-facing quiz UI, attempt recording, scoring, result view. Admin + tutor ranking/analytics also unbuilt. |
