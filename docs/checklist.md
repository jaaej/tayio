...
**Term test + leaderboard** - `/student/term-tests/[id]`, `/parent/term-tests/[id]/[childId]`, admin/tutor quiz maker (`docs/changes/2026-07-29-term-test-leaderboard.md`)
- [x] Typecheck clean
- [x] Unit tests pass (62/62 across 9 files, incl. 17 term-test cases)
- [x] Production build passes
- [x] Migrations 0028 and 0029 applied to the connected Supabase database
- [x] `db:check-rls` passes with the two new tables covered
- [x] Code review confirms `getStudentTermTest`'s option projection never selects `is_correct`
- [ ] Admin or tutor authors a term test for a subject and term, accepts or edits the release date, and approves it
- [ ] Student takes the term test once and is blocked from a second attempt
- [ ] Before release, the student (and the child's parent) sees only "results pending" - no score, rank, or answers
- [ ] At release, the student sees score, rank, and corrections; a no-show student appears at zero and can still review it
- [ ] The leaderboard shows the subject-and-term cohort ranked, with the viewer highlighted
- [ ] A parent views their child's result and board, read-only, gated by the same embargo
- [ ] Weekly quizzes remain unchanged in behavior and appearance
...
| End-of-term term test + separate leaderboard | 🔶 | ✅ | **Feature complete end-to-end 2026-07-29** (9-task plan, `docs/superpowers/plans/2026-07-29-term-test-leaderboard.md`; full record at `docs/changes/2026-07-29-term-test-leaderboard.md`). A term test is a `quizzes` row with `kind='term_test'` (migration `0028_term_test.sql`: `quiz_kind` enum, nullable `subject_week_id`, `term_id`/`results_release_at`, a partial-unique `(subject_id, term_id) where kind='term_test'`, and a check constraint enforcing the weekly-vs-term-test column shape), scored and term-scoped instead of week-scoped and unscored. `term_test_attempts`/`term_test_answers` (also 0028) store one attempt per student (`unique(quiz_id, student_id)`) and their per-question selections. Grading is server-only: `getStudentTermTest` (`src/lib/quiz-queries.ts`) strips `is_correct` from the take payload (verified by reading its option projection - only `id/questionId/text/position`), and `submitTermTest` (`src/app/_actions/term-tests.ts`) loads the answer key, calls the pure `gradeTermTest` (`src/lib/term-test.ts`, 17 unit tests), and persists the attempt + answers in one transaction, catching the unique-violation (`isUniqueViolation`, relocated to `src/lib/db-errors.ts`) as "already taken." Migration `0029_term_test_rls_tighten.sql` tightens the two new tables' student RLS from `for all` to SELECT-only, since every write goes through server code that bypasses RLS and a `for all` policy would let a student self-insert a fake leaderboard score via direct PostgREST. `getStudentQuiz` and `gradePracticeQuiz` (`src/app/_actions/quizzes.ts`) both gained a `kind === "weekly"` guard so an approved term test can never be opened or graded through the untouched weekly practice path. Results are embargoed until `results_release_at`: `deriveTermTestState` derives not_open/open/submitted_pending/released, and `getStudentTermTestResults`/`getParentTermTestResults` (`src/lib/term-test-results.ts`) check the release gate before touching the board, score, or answer key at all. The cohort (who is on the board and who scores zero as a no-show) is computed, not stored - `getTermTestCohort` selects students with an `enrollments` row in a class of the subject, active at the deadline (`enrolled_at <= results_release_at AND (withdrawn_at IS NULL OR withdrawn_at > results_release_at)`), since `classes` has no `term_id`; `rankTermTestBoard` left-joins the cohort with `term_test_attempts` so a no-show renders at score 0 and ranks last without a fabricated row. Routes: admin/tutor authoring reuses the existing quiz maker with a "Term tests" grouping and a release-date editor; student `/student/term-tests/[id]` (take/pending/results states + `src/components/term-test/leaderboard.tsx`) with an entry card on `/student/subjects/[id]`; parent `/parent/term-tests/[id]/[childId]` (read-only, family-link + cohort gated) with an entry card on `/parent/subjects/[id]` gated on release. Machine-verified 2026-07-29: both migrations applied to the connected database, `npm run db:check-rls` green, `npm run typecheck` green, `npm test` 62/62 across 9 files, `npm run build` exit 0 - so BE is ✅. **FE stays 🔶: no owner browser click-through yet** (authoring a term test, taking it once and being blocked on a second attempt, the pending-then-released transition, the leaderboard, and the parent view are all unverified in a real browser). |
