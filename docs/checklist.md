...
| Free-trial tracking | 🔶 | ✅ | Record a trial start + end date on an enrollment (migration 0030: `enrollments.trial_starts_at`/`trial_ends_at`, nullable, both-or-neither + start<=end check constraint). Tutor can see if a student is on a free trial. [INFRA] automated notification if a free-trial student misses a class - not built, no scheduler exists. [INFRA] when a trial ends, a notification prompts the admin to manually send the follow-up - not built; the `/admin/trials` "ended" bucket is the non-scheduler substitute. Admin can set/clear a per-student trial window from the class enrolled-students list (`src/app/admin/classes/[id]/_components/enrollments-manager.tsx`) and work the list from `/admin/trials` (`src/app/admin/trials/page.tsx` + `_components/trial-actions.tsx`), with a "Trials" nav entry in `src/components/admin/shell.tsx`; `setTrialDates` (`src/app/admin/_lib/actions-enrollments.ts`) is admin-only, Zod-validated, and `withActor`-audited. Tutor-facing "On trial" / "Trial ended" pills render via shared `src/components/tutor/trial-badge.tsx` on `/tutor/students`, `/tutor/students/[id]`, `/tutor/classes/[id]/students`, and `/tutor/lessons/[id]`'s attendance roster (substituted for `/tutor/attendance`, which has no per-student rows - see task-5-report.md); never shown to students or parents. `deriveTrialStatus`/`isEndingSoon` (`src/lib/trials.ts`, 9 unit tests) and `getTrials()` (`src/app/admin/_lib/queries.ts`) power both. Machine-verified 2026-07-29: migration 0030 applied to the connected database, `db:check-rls` green, typecheck green, `npm test` green, `npm run build` exit 0. See `docs/changes/2026-07-29-free-trials.md`. Still partial: missed-class / trial-ended auto-notifications not built ([INFRA]), and owner browser QA of the admin trial control, `/admin/trials`, and the tutor badges has not happened yet. |
