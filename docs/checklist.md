...
| Free-trial tracking | 🔶 | 🔶 | Record a trial start + end date. Tutor can see if a student is on a free trial. [INFRA] automated notification if a free-trial student misses a class. When a trial ends, a notification prompts the admin to manually send the follow-up (the message itself is not automated). Admin can now set/clear a per-student trial window from the class enrolled-students list (`src/app/admin/classes/[id]/_components/enrollments-manager.tsx`) and work the list from `/admin/trials` (`src/app/admin/trials/page.tsx` + `_components/trial-actions.tsx`), with a "Trials" nav entry in `src/components/admin/shell.tsx`. `deriveTrialStatus`/`isEndingSoon` (`src/lib/trials.ts`) and `getTrials()` (`src/app/admin/_lib/queries.ts`) power both. typecheck + build pass (2026-07-29). Still partial: tutor-facing trial badge not built, and the missed-class / trial-ended notifications are not built. |
