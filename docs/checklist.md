...
| `admin_restricted` (reception) | Lower-tier admin with no access to sensitive financials | 🔶 | ✅ | **First real per-feature gating built 2026-08-01.** `isUnrestrictedAdmin`/`adminTier` (`src/lib/roles.ts`) + `requireUnrestrictedAdmin` (`src/lib/auth.ts`) now gate the built privileged surface: reception cannot create admin/tutor accounts or change a user's role (`actions-users.ts`, server-enforced + role options hidden/disabled in the create/edit forms), and `/admin/revenue` + `/admin/settings` (PIN management) redirect reception to the dashboard. Financials stay behind the PIN wall too (migrations 0020/0021). Predicates unit-tested (`roles.test.ts`). Matrix rows for features that don't exist yet (refund, invoice-creation, discount, bulk export) are moot until those ship - they just call the same helper. FE stays 🔶 pending browser click-through as each tier. |
| `admin_unrestricted` (owner) | Full admin with all access | 🔶 | ✅ | Same helper. Owner keeps full access; role-change + account-deactivation now additionally require the PIN step-up (`isAdminUnlocked`, unlock in `/admin/settings`, ~30 min), so the settings copy is now accurate. See [Role Tiering](#role-tiering-student--admin). |
