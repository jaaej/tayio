...
| **(extra) Subject deep-dive** | Per-subject mastery + topic list | 🔶 | ✅ | `/student/subjects/[id]`. **Curriculum restyle (2026-07-28):** neutral-canvas / colour-as-spotlight redesign. New client `curriculum-shell.tsx` owns active-week state, shallow-syncs `?week=`, and cross-fades content; a new client `week-rail.tsx` replaces the old static week strip with a slim 56px collapsible rail that hover-expands as an overlay and can be pinned (persisted to `localStorage`); `week-content.tsx` restyled to neutral cards with colour reserved for the header tile, active week, and progress. All weeks for the term are preloaded server-side, so week switching is an instant client swap with no per-week fetch; term switching stays a server navigation. Dead rails `week-strip.tsx` and `week-sidebar.tsx` removed. Typecheck + build pass; FE stays 🔶 pending owner browser verification (rail hover/pin, cross-fade, deep links, reduced-motion, 375px) - see `docs/changes/2026-07-28-curriculum-restyle.md`. |
