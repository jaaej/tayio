...
| Reschedule slot picker hides already-booked slots | ⬜ | ⬜ | **Follow-up (recommended, flagged 2026-07-30).** The reschedule/redeem slot picker shows the tutor's declared availability without subtracting slots already occupied by a lesson, so it can offer a taken slot that the double-booking guard then rejects on submit ("that slot was just taken"). Recommended fix (Option 1): in the availability layer (`src/lib/availability.ts`, `expandAvailability`/`getOneOnOneSlots`) drop any candidate slot overlapping an existing lesson for that tutor, so the picker only shows genuinely-free slots. Read-side, no schema change; benefits student/parent reschedule, admin one-off, and credit redemption. Group-session capacity booking is explicitly out of scope - self-serve reschedule creates an individual make-up against the tutor's time, so the tutor is fully occupied during any lesson (group or 1-on-1). |
