...
| Discontinued-students tab (admin users) | 🔶 | ✅ | `/admin/users` now has an "All accounts" / "Discontinued" tab bar (`?tab=discontinued`). Query `getDiscontinuedStudents()` in `src/app/admin/_lib/queries.ts` (grouped by student); rendered in `src/app/admin/users/page.tsx`. **Consolidated 2026-08-03:** the redundant standalone `/admin/leaving` page was deleted and its "Students leaving →" header link removed - the Discontinued tab is now the single home for withdrawn students. typecheck + build pass; browser verification owner-pending. |
