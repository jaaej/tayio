...
const TIERS: Tier[] = [
  { key: "sprint", label: "Sprint", accent: "#1fa974", fg: "#0e7a4d", icon: Rabbit },
  { key: "easy", label: "Easy", accent: "#2e8fd6", fg: "#1e6fb0", icon: Sparkles },
  { key: "medium", label: "Medium", accent: "#f58a07", fg: "#b5610a", icon: Flame },
  { key: "hard", label: "Hard", accent: "#f2616b", fg: "#cc3a45", icon: Swords },
  { key: "genius", label: "Genius", accent: "#7b5bd6", fg: "#5b3fb0", icon: Brain },
];
        ...
        {TIERS.map((t, i) => {
          ...
          return (
            <button
              ...
              className={`group relative overflow-hidden text-left rounded-[22px] p-5 pt-6 cursor-pointer transition-all duration-200 hover:-translate-y-1.5 hover:shadow-xl active:scale-[0.98] active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-brand-500 ${
                special ? "blitz-glow text-ink" : "blitz-pop border border-line bg-surface"
              }`}
              style={
                special
                  ? {
            ...
            >
              {special ? (
                <>
                  <span className="absolute -right-6 -top-8 h-24 w-24 rounded-full bg-white/40 blur-xl blitz-float" />
              ...
              <div
                className="relative h-14 w-14 grid place-items-center rounded-[16px] transition-transform duration-200 group-hover:scale-110 group-hover:-rotate-6"
                style={
                  special
              ...
              <div className="relative mt-4 text-[19px] font-extrabold text-ink">
                {t.label}
              </div>
              <div
                className="relative mt-3 inline-flex items-center rounded-full px-3 py-1.5 text-[12px] font-bold tabular-nums"
                style={
                  special
                    ? { backgroundColor: "rgba(255,255,255,0.5)", color: "#065f46" }
                    : { backgroundColor: `${t.accent}1f`, color: t.fg }
