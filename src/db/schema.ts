...
export const tutorLeaveStatusEnum = pgEnum("tutor_leave_status", [
  "pending",
  "approved",
  "rejected",
  "cancelled",
...
export const tutorCoverStatusEnum = pgEnum("tutor_cover_status", [
  "open",
  "claimed",
  "cancelled",
  "expired",
 ...
 * lessons are published as individual shifts on the tutor cover board.
 */
export const tutorLeaveRequests = pgTable(
  "tutor_leave_requests",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tutorId: uuid("tutor_id")
      .notNull()
    ...
    reason: text("reason").notNull(),
    status: tutorLeaveStatusEnum("status").notNull().default("pending"),
    decidedById: uuid("decided_by_id").references(() => profiles.id),
    decidedAt: timestamp("decided_at", { withTimezone: true }),
    lastUncoveredReminderOn: date("last_uncovered_reminder_on"),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
  ...
  (t) => [
    index("tutor_leave_requests_tutor_idx").on(t.tutorId),
    index("tutor_leave_requests_status_dates_idx").on(
      t.status,
      t.startDate,
      t.endDate,
    ),
...
/** One lesson needing another tutor, either standalone or generated by leave. */
export const tutorCoverRequests = pgTable(
  "tutor_cover_requests",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    lessonId: uuid("lesson_id")
      .notNull()
    ...
    originalTutorId: uuid("original_tutor_id")
      .notNull()
      .references(() => profiles.id),
    replacementTutorId: uuid("replacement_tutor_id").references(
      () => profiles.id,
    ),
    reason: text("reason").notNull(),
    status: tutorCoverStatusEnum("status").notNull().default("open"),
    claimedAt: timestamp("claimed_at", { withTimezone: true }),
    alert48SentAt: timestamp("alert_48_sent_at", { withTimezone: true }),
    alert24SentAt: timestamp("alert_24_sent_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
  ...
  (t) => [
    uniqueIndex("tutor_cover_requests_lesson_idx").on(t.lessonId),
    index("tutor_cover_requests_status_idx").on(t.status),
    index("tutor_cover_requests_leave_idx").on(t.leaveRequestId),
    index("tutor_cover_requests_replacement_idx").on(t.replacementTutorId),
  ],
);
...
export const notifications = pgTable(
  "notifications",
  {
    ...
    body: text("body"),
    href: text("href"),
    // Stable event key for cron/retry-safe delivery. Null for ordinary inbox
    // messages; unique per recipient when supplied by a scheduled workflow.
...
export type TutorLeaveRequest = typeof tutorLeaveRequests.$inferSelect;
export type TutorCoverRequest = typeof tutorCoverRequests.$inferSelect;
